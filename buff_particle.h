...
class buff_particles : public BLIB::particles {
	...
	buff_particles* clone_impl() const override { return new buff_particles(); }
	...
	void update_each(float elapsed_time, particle& particle) override {
		...
		particle.p.position.y = min_height + max_height * ((t < 0.5f) ? (8 * t * t * t * t) : 1 - powf(-2 * t + 2, 4) * 0.5f);
...
public:
	buff_particles() { load_texture("particles/green_star.png"); }
};
